class DeliveriesController < ApplicationController
  ...
  def index
    ...
    @list_of_deliveries = matching_deliveries.order({ :supposed_to_arrive_on => :asc }).where({ :arrived => false}).where({ :user_id => current_user.id})

    @list_of_deliveries_received = matching_deliveries.order({ :updated_at => :desc }).where({ :arrived => true}).where({ :user_id => current_user.id})
